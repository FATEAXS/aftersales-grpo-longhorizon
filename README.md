...
`src/aftersales_grpo/simulator/generator.py` 用固定种子生成全部数据。先造
世界:8 个类目的售后政策、45 个商品、600 个用户、1816 个带完整时间线的
订单。再在世界上出题:按场景权重抽类型,挑商品、造用户和订单,时间线与
场景严格对齐(超期投诉对应的订单就是 30 天前签收),加干扰订单,从模板
渲染用户开场白,最后冻结评分用的 TaskFacts。种子固定,任何人重跑得到
逐字节相同的数据,manifest 里存了各文件 SHA-256。

TaskFacts(该不该拒绝、退款上限、期望解法)只有环境评分时用,模型只能
看到任务卡(task_id 和用户开场白)。训练 400 / 验证 100 / 评测 100,
task_id 零重叠。
...
**创新一:SOP 过程奖励(SOP-PR)**。售后处理有固定作业流程:查订单、
核政策、看风控、执行动作、回复用户、如实结案。每一步都能用规则从轨迹里
检查,首次达成给 0.05,如实结案给 0.10。GRPO 奖励为终局奖励加 λ 倍过程
奖励。出现错订单、幻觉订单或政策违规时过程奖励置 0。

**创新二:风险加权优势与组内最优基线**。拒绝/风控类任务上,错误动作的
优势按 1.5 倍放大;组内存在成功轨迹时,以其中最优奖励为基线计算组内优势
(而不是均值),避免"大多数都错"的组把错误当成正常水平。

**工程配套一:澄清回合(环境机制)**。用户名下有两笔同名商品订单且没有
给订单号时,环境里的用户模拟器会回答 Agent 的追问(告知订单号)。不追问
直接操作:选对订单扣 0.15,选错订单按硬门槛扣 0.6。

**工程配套二:红线协议(采样策略)**。90 天退款 6 次以上的用户属于风控,
其无理由退款诉求必须转人工。这类任务在训练采样中加倍,评测单列合规面板。

**创新带来的提点**:同样的 40 步训练,均匀采样的 GRPO 在"应拒绝却违规
办理"上纹丝不动(56% → 56%);加上两个创新和配套机制后,成功率保持 82%
不变,违规办理率降到 32%(-24 个百分点)。λ 调到 1 可以进一步压到 28%,
代价是成功率降 5 个点。
